              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    ElevatedButton(onPressed: () => _setMode(true), child: const Text('AUTO')),
                    const SizedBox(width: 8),
                    OutlinedButton(onPressed: () => _setMode(false), child: const Text('MANUAL')),
                    const SizedBox(width: 12),
                    Chip(
                      label: Text(s?.autoMode == true ? 'AUTO' : 'MANUAL'),
                      backgroundColor:
                          s?.autoMode == true ? Colors.green.shade100 : Colors.red.shade100,
                    ),
                  ],
                ),
                ...
                Wrap(
                  ...
                  children: [
                    _deviceCard('AirCon', 0,
                        (s?.ledStatus.length ?? 0) > 0 && s!.ledStatus[0] == 1, s?.autoMode == true),
                    _deviceCard('Heater', 1,
                        (s?.ledStatus.length ?? 0) > 1 && s!.ledStatus[1] == 1, s?.autoMode == true),
                    _deviceCard('Dehumid', 2,
                        (s?.ledStatus.length ?? 0) > 2 && s!.ledStatus[2] == 1, s?.autoMode == true),
                  ],
                ),
              ],
            ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(on ? 'ON' : 'OFF', style: TextStyle(color: on ? Colors.green : Colors.red)),
            const SizedBox(height: 8),
            Row(
              mainAxisSize: MainAxisSize.min,
              children: [
            ...
            if (autoLocked)
              const Padding(
                ...
                child: Text('AUTO mode', style: TextStyle(fontSize: 12, color: Colors.grey)),
              ),
          ],
        ),
      ),
